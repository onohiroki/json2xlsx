...
2. `json2xlsx` がその JSON を読み取り，`.xlsx` に変換する．[5]

全体を通して JSON は SheetJS 風を扱いますが，外部ツールとのデータやり取りの利便性のために，二次元配列形式の JSON の入力も受け付けます．

この分離により，Go ツールは軽量・テスト容易・OSS 公開しやすい構成になります．
...
- 基本表，数式，改行，枠線，色，数値書式，リンク，固定ペインなどをサポート（二次元配列形式は数式やスタイルには非対応）．
- AI 生成部を切り離しているため，任意の LLM と組み合わせやすい．
- **グラフ生成** — 棒・折れ線・円・散布図など 8 種類のチャートをサポート．タイトル・凡例・軸・データラベルも設定可能．
- **画像埋め込み** — ファイルパスまたは base64 データからシートに画像を埋め込み可能．シートの背景画像にも対応．
...
`--date-rfc3339` を指定すると RFC3339 (UTC) 文字列を `v` に出力します（ラウンドトリップ可能）．
`--date-serial` を明示的に指定してもデフォルトと同じ動作です（互換性のためのエイリアス）．
時刻だけの値 (`9:05`) は **日付なしの時刻** として扱います．

`--date-display` は **非推奨** です．Excel の表示文字列を出力しますが，日付表示形式はロケール依存のため `to-xlsx` で確実にラウンドトリップできません．
ラウンドトリップ可能な人読可能な出力には `--date-rfc3339` を，人読表示には `to-md` / `to-html` を使用してください．

**画像出力モード** — `--image-mode` で XLSX 内の画像の出力方法を制御します：
- `base64`（デフォルト）: JSON 出力内に base64 エンコードされた画像データを `data` フィールドとして埋め込みます．
- `file`: 画像を個別のファイル（`image_1.png`，`image_2.jpg` など）として JSON 出力と一緒に保存します．JSON 内の `path` フィールドがファイルを参照します．
...
**文字列リテラル:** `"Hello"` (ダブルクォートで囲む．`""` はエスケープで 1 つの `"` を表す)

**連結演算子:** `&`

**算術演算子:** `+`, `-`, `*`, `/`

**比較演算子:** `<`, `>`, `=`, `<=`, `>=`, `<>` (等しくない)．比較結果は `1` (真) または `0` (偽) です．文字列同士は大文字小文字を区別せず比較します．型混在時は「数値 < 文字列」の順序規則に従います (Excel 準拠)．
...
| `CONCATENATE(str1, str2, ...)` | CONCAT の別名 |
| `LEFT(str, n)` | 先頭から `n` 文字 (rune 単位，日本語対応) |
| `RIGHT(str, n)` | 末尾から `n` 文字 (rune 単位，日本語対応) |
| `MID(str, start, n)` | `start` 位置から `n` 文字 (1 始点，rune 単位) |
| `LEN(str)` | 文字数 (rune 単位，日本語対応) |
| `UPPER(str)` | 大文字に変換 |
| `LOWER(str)` | 小文字に変換 |
...
日付セル (`t: "d"`) は自動的に人読可能な文字列に変換されます（シリアル値 → RFC3339 形式，文字列値 → そのまま）．
...
シート内の `pictures` 配列で画像を埋め込めます．各画像はファイルパスまたは base64 データで指定できます．シートレベルの背景画像は `background` フィールドで設定します．

### Picture オブジェクトのフィールド

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `cell` | string | アンカーセル（例: `"B1"`） |
| `path` | string | 画像ファイルのパス（相対/絶対） |
| `data` | string | base64 エンコードされた画像データ（`path` の代替） |
| `extension` | string | ファイル拡張子（`png`，`jpg`，`gif`）— `data` 使用時に必須 |
| `altText` | string | 代替テキスト |
| `offsetX` | int | アンカーセルからの水平オフセット（EMU） |
| `offsetY` | int | アンカーセルからの垂直オフセット（EMU） |
| `scaleX` | float | 水平方向の拡大率 |
| `scaleY` | float | 垂直方向の拡大率 |
| `positioning` | string | `"oneCell"`（セルと共に移動/サイズ変更）または `"absolute"` |
| `hyperlink` | string | ハイパーリンク URL |
| `printObject` | bool | 画像を印刷するか |
| `locked` | bool | 画像をロックするか |
| `lockAspectRatio` | bool | 縦横比を固定するか |

### Background オブジェクトのフィールド

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `path` | string | 画像ファイルのパス |
| `data` | string | base64 エンコードされた画像データ（`path` の代替） |
| `extension` | string | ファイル拡張子 — `data` 使用時に必須 |
...
完全な例は `samples/pictures.json` および `samples/pictures_new.json` を参照してください．
...
| 数式 | Yes | セル参照式を想定 |
| セル内改行 | Yes | `\n` を改行として扱う |
| 枠線 | Yes | thin / medium など |
| 背景色 | Yes | Fill を利用 |
| 数値書式 | Yes | `z` / `numFmt` 相当 [6] |
| ハイパーリンク | Yes | `L` フィールドで指定 |
| マージセル | Yes | `merges` 配列で指定 |
| グラフ | Yes | 棒，折れ線，円，散布図など 8 種 |
| 画像埋め込み | Yes | ファイルパスまたは base64，シート背景画像 |
