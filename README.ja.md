...
- 基本表，数式，改行，枠線，色，数値書式，リンクなどをサポート（二次元配列形式は数式やスタイルには非対応）．[6][9][10]
...
`excelize` は Go で Excel ファイルを読み書きする代表的な OSS ライブラリ，`jsonschema` は JSON Schema バリデーションに，`go-runewidth` は Markdown 出力時の文字幅計算に使用しています．[2][1]
